...
//	public String getvNum(int index) {
//		cu.
//		return null;
//	}
	...
	public List<String> getDefs(int index) {
		...
		if (index >= instructions.length) return result;
		SSAInstruction insn = instructions[index];
//		System.out.println(insn + ":from:"+index);
		if (insn == null) return result;
	...
	public List<String> getUses(int index) {
		...
		if (index >= instructions.length) return result;
		SSAInstruction insn = instructions[index];
		ISSABasicBlock queryBlock = LocalLiveRangeAnalysis.findBlock(ir, index);
//		System.out.println(insn + ":from:"+index);
		if (insn == null) return result;
	...
	//added by shengzhe
	public List<String> getParas(int index) {
		List<String> result = new ArrayList<String>();
		SSAInstruction insn = instructions[index];
		if (insn instanceof SSAAbstractInvokeInstruction) {
			SSAAbstractInvokeInstruction this_insn = (SSAAbstractInvokeInstruction) insn;
			int num = this_insn.getNumberOfParameters();
			int vnums[] = this.symTab.getParameterValueNumbers();
//			for(int pn : vnums) {
////		        System.out.println("parnum: " + pn + " " + this.symTab.getValueString(pn));
//		    }
//			int vNum = 0;
//			for (int i = 0; i < num; i++) {
//				vNum = this_insn.getParameterValueNumbers(i);
//				result.add(getName(index, vNum) + ":" + vNum);
//			}
		}
		return result;
	}
...
//		}
		...
		try {
			name = sourceNames[vNum];
//			System.out.println("find-source:" + name[0]);
		} catch(Exception e) {
			name = null;
		}
		String result = null;
		...
		if (name.length != 0 ) {
			result = name[0];
//			System.out.println("find-local:" + result);
		}else if (result == null) {
			result = "v" + vNum;
		}								
		return result;
	...
	private InsnNode createInsnNode(SSAInstruction insn, Subgraph g) {
		//insn null checker
		if (insn == null) {
			System.out.println("null insn created");
			return null;
		}
		//end
