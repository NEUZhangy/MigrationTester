...
public class JavaASTHelper implements ASTHelper<JavaStructureNode> {
    ...
    public Node createMethodBodyTree(JavaStructureNode node) {
        ...
        else if(astNode instanceof AbstractVariableDeclaration) {
        	Node root = createRootNode(node, astNode);
        	fBodyConverter.initialize(root, astNode, fComments, fCompilation.getScanner());
            return root;
        }
...
//        if (builder.get_init_start() != -1) {
//        	String fn = cu.getFileName().toString();
//        	appendMethodA(fn, builder.get_init_str(), builder.get_init_start());
//        	System.out.println("");
//        }
//        JavaStructureNode node = new JavaStructureNode(Type.METHOD, null, name, builder.subcu);
//        JavaStructureTreeBuilder builder = new JavaStructureTreeBuilder(node);
//        cu.traverse(builder, (CompilationUnitScope) null);
    ...
    public SourceCodeEntity createSourceCodeEntity(JavaStructureNode node) {
    	// changed by shengzhe
    	if (!node.getName().contains("Initializer")) {
    ...
    public static void appendMethodA(String fileName, String content, int SeekLength) {
        try {
            RandomAccessFile randomFile = new RandomAccessFile(fileName, "rw");
            long fileLength = randomFile.length();
            randomFile.seek(SeekLength);
