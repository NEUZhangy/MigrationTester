...
public class JavaSourceCodeChangeClassifier implements SourceCodeChangeClassifier {
    ...
    private static final String libStr = CommonValue.possible_lib_name1;
    ...
    private Map<String, String> imp_pattern;
    private Map<String, String> ano_pattern;
    ...
    private SourceCodeChange classify(Move move) {
        ...
        if ((move.getParentEntity().getType() == JavaEntityType.SUPER_CLASS_TYPES 
        		|| move.getParentEntity().getType() == JavaEntityType.SUPER_INTERFACE_TYPES)
        		&& (move.getNewParentEntity().getType() == JavaEntityType.SUPER_CLASS_TYPES
        		|| move.getNewParentEntity().getType() == JavaEntityType.SUPER_INTERFACE_TYPES)) {
        	result = handleInterfacetoSuperClassSignatureChange(move);
        }
    ...
    // Added by SX
    private SourceCodeChange handleInterfacetoSuperClassSignatureChange(Move move) {
    	if (!move.getRootEntity().getUniqueName().contains(libStr)
    			&& move.getRootEntity().getUniqueName().contains(CommonValue.possible_lib_name2)) {
    		return null;
    	}
    	if (move.getParentEntity().getType() == JavaEntityType.SUPER_CLASS_TYPES
				&& move.getNewParentEntity().getType() == JavaEntityType.SUPER_INTERFACE_TYPES) {
            System.out.println(move.getRootEntity().getUniqueName());
			System.out.println("implements " + move.getChangedEntity().getUniqueName()
					+ " -> " + "extends " + move.getNewEntity().getUniqueName());
			// modified by shengzhe 4/5 18
//			imp_pattern.put(CommonValue.common_commit_number, "implements " + move.getChangedEntity().getUniqueName()
//					+ " -> " + "extends " + move.getNewEntity().getUniqueName() );
			DatabaseControl data1 = new DatabaseControl();
			int label = data1.insertpattern(("implements " + move.getChangedEntity().getUniqueName()),
					("extends " + move.getNewEntity().getUniqueName()), CommonValue.common_old_version, CommonValue.common_new_version, "Superclass Changes", String.valueOf(1));
			data1.insertsnippet(move.getChangedEntity().toString(), move.getNewEntity().toString(), CommonValue.common_project_name, CommonValue.common_commit_number, String.valueOf(label)
					, CommonValue.common_old_version+"-"+CommonValue.common_new_version);
			move.setChangeType(ChangeType.STATEMENT_PARENT_CHANGE);
    	}
		else if (move.getParentEntity().getType() == JavaEntityType.SUPER_INTERFACE_TYPES
				&& move.getNewParentEntity().getType() == JavaEntityType.SUPER_CLASS_TYPES) {
	        System.out.println(move.getRootEntity().getUniqueName());
			System.out.println("extends " + move.getChangedEntity().getUniqueName()
					+ " -> " + "implements " + move.getNewEntity().getUniqueName());
//			imp_pattern.put(CommonValue.common_commit_number, "extends " + move.getChangedEntity().getUniqueName()
//					+ " -> " + "implements " + move.getNewEntity().getUniqueName() );
			DatabaseControl data1 = new DatabaseControl();
			int label = data1.insertpattern(("extends " + move.getChangedEntity().getUniqueName()),
					("implements " + move.getNewEntity().getUniqueName()), CommonValue.common_old_version, CommonValue.common_new_version, "Superclass Changes", String.valueOf(1));
			data1.insertsnippet(move.getChangedEntity().toString(), move.getNewEntity().toString(), CommonValue.common_project_name, CommonValue.common_commit_number, String.valueOf(label)
					, CommonValue.common_old_version+"-"+CommonValue.common_new_version);
			move.setChangeType(ChangeType.STATEMENT_PARENT_CHANGE);
		}
    ...
    private SourceCodeChange handleFieldDeclarationChange(Update update) {
        ...
        } else if (update.getChangedEntity().getType() == JavaEntityType.QUALIFIED_NAME) {
    ...
    private SourceCodeChange extractRenaming(Update update) {
        ...
        } else if (update.getNewEntity().getType() == JavaEntityType.CLASS_INSTANCE_CREATION) {
        	// added by shengzhe April-2
            update.setChangeType(ChangeType.ATTRIBUTE_RENAMING);
        } else if (update.getNewEntity().getType() == JavaEntityType.SINGLE_TYPE) {
        	// added by shengzhe April-5
